...
impl From<Error> for u8 {
	fn from(value: Error) -> Self {
		match value {
			...
			Error::InvalidSignatureTime => 5,
			Error::InvalidDidKeyRevealed => 6,
		...
		DidMerkleHasher: Hash<Output = Commitment>,
	{
		...
		cfg_if::cfg_if! {
			if #[cfg(feature = "runtime-benchmarks")] {
				...
				let _ = verify_trie_proof::<LayoutV1<DidMerkleHasher>, _, _, _>(
					&self.dip_commitment,
					self.dip_proof.blinded.as_slice(),
					&proof_leaves_key_value_pairs,
				);
...
impl<KiltDidKeyId, KiltAccountId, KiltBlockNumber, KiltWeb3Name, KiltLinkableAccountId, ConsumerBlockNumber>
	DipDetailsAndUnverifiedDidSignatureTime<
		KiltDidKeyId,
		KiltAccountId,
		KiltBlockNumber,
	...
	KiltBlockNumber: BenchmarkDefault,
{
	...
	pub fn retrieve_signing_leaf_for_payload(
		self,
		payload: &[u8],
	) -> Result<DipVerifiedInfo<KiltDidKeyId, KiltAccountId, KiltBlockNumber, KiltWeb3Name, KiltLinkableAccountId>, Error>
	{
		...
		let maybe_signing_key_entry = revealed_verification_keys
			.enumerate()
		...
		cfg_if::cfg_if! {
			if #[cfg(feature = "runtime-benchmarks")] {
				let signing_key_entry = maybe_signing_key_entry.unwrap_or_else(|| {
					(0, &RevealedDidMerkleProofLeaf::from(RevealedDidKey { id: KiltDidKeyId::default(), details: DidPublicKeyDetails { key: DidVerificationKey::Ed25519(sp_core::ed25519::Public::from_raw([0u8; 32])).into(), block_number: KiltBlockNumber::default() }, relationship: DidVerificationKeyRelationship::Authentication.into() }))
				});
			} else {
				let signing_key_entry = (index.saturated_into(), self.revealed_leaves);
			}
		}

		Ok(DipVerifiedInfo {
			revealed_leaves: signing_key_entry.1,
			signing_leaf_index: signing_key_entry.0,
		})
	}
}
