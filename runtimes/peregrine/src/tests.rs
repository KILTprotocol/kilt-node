...
use did::DeriveDidRuntimeCallAuthorizationVerificationKeyRelationship;
...
use super::{RuntimeCall, Runtime};

#[test]
fn call_size() {
	assert!(
		core::mem::size_of::<RuntimeCall>() <= 272,
		"size of RuntimeCall is {:?} bytes which is more than 240 bytes: some calls have too big arguments, use Box to reduce the size of RuntimeCall.
...
fn test_derive_did_verification_relation_ctype() {
	let c1 = RuntimeCall::Ctype(ctype::RuntimeCall::add {
		ctype: vec![0, 1, 2, 3],
	});
	let c2 = RuntimeCall::Ctype(ctype::RuntimeCall::add {
		ctype: vec![0, 1, 2, 3, 3],
	});
	let c3 = RuntimeCall::Ctype(ctype::RuntimeCall::add {
		ctype: vec![0, 1, 2, 3, 3],
	});
	let c4 = RuntimeCall::Ctype(ctype::RuntimeCall::add {
		ctype: vec![0, 1, 2, 100],
	});

	let cb = RuntimeCall::Utility(pallet_utility::RuntimeCall::batch {
...
fn test_derive_did_key_web3name() {
	assert_eq!(
		RuntimeCall::Web3Names(pallet_web3_names::RuntimeCall::claim {
	...
	assert_eq!(
		RuntimeCall::Web3Names(pallet_web3_names::RuntimeCall::release_by_owner {}).derive_verification_key_relationship(),
		Ok(did::DidVerificationKeyRelationship::Authentication)
	);
}
...
fn test_derive_did_key_lookup() {
	assert_eq!(
		RuntimeCall::DidLookup(pallet_did_lookup::RuntimeCall::associate_account {
	...
	assert_eq!(
		RuntimeCall::DidLookup(pallet_did_lookup::RuntimeCall::remove_account_association {
...
fn test_derive_did_verification_relation_fail() {
	...
	let c3 = RuntimeCall::System(frame_system::RuntimeCall::remark {
	...
	assert_eq!(
		cb.derive_verification_key_relationship(),
		Err(did::RelationshipDeriveError::NotRuntimeCallableByDid)
	);
}
...
fn test_derive_did_verification_relation_nested_fail() {
	...
	let f3 = RuntimeCall::System(frame_system::RuntimeCall::remark {
		remark: vec![0, 1, 2, 3, 3],
	});
	let c4 = RuntimeCall::Ctype(ctype::RuntimeCall::add {
		ctype: vec![0, 1, 2, 100],
...
fn test_derive_did_verification_relation_single() {
	...
	let cb = RuntimeCall::Utility(pallet_utility::RuntimeCall::batch { calls: vec![c1] });
...
fn test_derive_did_verification_relation_empty() {
	let cb = RuntimeCall::Utility(pallet_utility::RuntimeCall::batch { calls: vec![] });
