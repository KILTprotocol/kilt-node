...
pub mod proof_generation {
		...
		for DidPalletMerkleRootHasher<T>
	{
		...
		) -> Result<DidMerkleRootHasherOutput<T>, Self::Error> {
	...
	impl<T: did::Config> DidPalletMerkleRootHasher<T> {
		fn calculate_root_with_db(
		...
		) -> Result<DidMerkleRootHasherOutput<T>, ()> {
			...
			// Should never happen
			trie_builder.insert(b"did", &did.encode()).map_err(|_| ())?;
			// Should never happen
			trie_builder
				.insert(b"auth", &details.authentication_key.encode())
				.map_err(|_| ())?;
			if let Some(att_key) = details.attestation_key {
				trie_builder.insert(b"att", &att_key.encode()).map_err(|_| ())?;
			}
			if let Some(del_key) = details.delegation_key {
				trie_builder.insert(b"del", &del_key.encode()).map_err(|_| ())?;
			}
			details
				.key_agreement_keys
				.iter()
				.enumerate()
				.try_for_each(|(i, k)| -> Result<(), ()> {
					trie_builder
		...
		pub fn generate_proof(
			...
			details: &DidDetails<T>,
			key_id: KeyIdOf<T>,
		) -> Result<(DidMerkleRootHasherOutput<T>, Vec<Vec<u8>>), ()> {
			let mut db = MemoryDB::default();
...
pub mod identity_retrieval {
	...
	impl<T: did::Config> IdentityProvider<DidIdentifierOf<T>, DidDetails<T>, ()> for DidPalletProvider<T> {
		type Error = ();
		fn retrieve(identifier: &DidIdentifierOf<T>) -> Result<Option<(DidDetails<T>, ())>, Self::Error> {
