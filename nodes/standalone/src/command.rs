	...
	match &cli.subcommand {
		...
		Some(Subcommand::TryRuntime(cmd)) => {
			...
			runner.async_run(|config| {
				...
				let info_provider = timestamp_with_aura_info(10_000);
				Ok((
					cmd.run::<kestrel_runtime::opaque::Block, sc_executor::sp_wasm_interface::ExtendedHostFunctions<
						sp_io::SubstrateHostFunctions,
						<crate::service::ExecutorDispatch as sc_service::NativeExecutionDispatch>::ExtendHostFunctions,
		...
		Some(Subcommand::TryRuntime) => Err("TryRuntime wasn't enabled when building the standalone-node binary. \
