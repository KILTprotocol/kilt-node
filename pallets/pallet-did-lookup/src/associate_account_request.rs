...
fn to_ss58(public_key: &[u8], prefix: u16) -> String {
	...
	let mut v = match ident {
		...
		64..=16_383 => {
			...
			let first = ((ident & 0b0000_0000_1111_1100).saturated_into::<u8>()).wrapping_shr(2);
			// lower two bits of the lower byte in the high pos,
			// lower bits of the upper byte in the low pos
			let second = ((ident.wrapping_shr(8)).saturated_into::<u8>())
				| ((ident & 0b0000_0000_0000_0011).saturated_into::<u8>()).wrapping_shl(6);
	...
	v.extend(public_key);
	let r = ss58hash(&v);
	let r_slice_option = r.get(0..2);
	let r_slice = r_slice_option.unwrap_or(&[]);
	v.extend(r_slice);
	v.to_base58()
}
