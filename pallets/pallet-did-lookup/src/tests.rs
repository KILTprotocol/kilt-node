...
fn test_reclaim_deposit_not_authorized() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				DidLookup::reclaim_deposit(RuntimeOrigin::signed(ACCOUNT_00), ACCOUNT_00),
				Error::<Test>::NotAuthorized
			);
			assert_eq!(
				Balances::reserved_balance(ACCOUNT_01),
...
fn test_update_deposit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(DidLookup::update_deposit(RuntimeOrigin::signed(ACCOUNT_00), ACCOUNT_00));
			assert_eq!(
				Balances::reserved_balance(ACCOUNT_00),
				<Test as crate::Config>::Deposit::get()
			);
...
fn test_update_deposit_unauthorized() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DidLookup::update_deposit(RuntimeOrigin::signed(ACCOUNT_01), ACCOUNT_00),
				Error::<Test>::NotAuthorized
			);
		})
}
