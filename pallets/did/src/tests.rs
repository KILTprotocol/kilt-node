...
use std::{convert::TryFrom, format};
...
fn check_successful_complete_update() {
	...
	println!("{:?}", old_att_key.public());
	let new_att_key = get_ed25519_attestation_key(false);
	println!("{:?}", new_att_key.public());
	...
	let mut old_verification_keys_map = BTreeMap::<Hash, did::PublicVerificationKey>::new();
	old_verification_keys_map.insert(
		generate_attestation_key_id(
			old_did_details.attestation_key.unwrap(),
			old_did_details.last_tx_counter,
		),
		old_did_details.attestation_key.unwrap(),
	);
	println!("{:?}", generate_attestation_key_id(
		old_did_details.attestation_key.unwrap(),
		old_did_details.last_tx_counter,
	));
	...
	// let hash_2 = generate_attestation_key_id(*stored_key_2, operation.tx_counter);
	// assert_eq!(*stored_key_id_2, hash_2);
...
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	old_did_details.attestation_key =
// Some(did::PublicVerificationKey::from(att_key.public())); 	old_did_details.
// delegation_key = Some(did::PublicVerificationKey::from(del_key.public()));

// 	// Remove both attestation and delegation key
// 	let mut operation = generate_base_did_update_operation(ALICE_DID);
// 	operation.attestation_key_update =
// did::DidVerificationKeyUpdateAction::Delete; 	operation.delegation_key_update
// = did::DidVerificationKeyUpdateAction::Delete; 	operation.tx_counter =
// old_did_details.last_tx_counter + 1u64;
...
// Auth key and key agreement key unchanged 	assert_eq!(new_did_details.auth_key,
// old_did_details.auth_key); 	assert_eq!(new_did_details.key_agreement_key,
// old_did_details.key_agreement_key); 	assert_eq!(new_did_details.
// attestation_key, None); 	assert_eq!(new_did_details.delegation_key, None);
...
// 			.collect()
// 	);
// 	assert_eq!(new_did_details.last_tx_counter, old_did_details.last_tx_counter +
// 1u64); }
...
// 		did::PublicVerificationKey::from(get_ed25519_attestation_key(false).
// public()), 		did::PublicVerificationKey::
// from(get_sr25519_attestation_key(true).public()), 		did::PublicVerificationKey:
// :from(get_sr25519_attestation_key(false).public()), 	];
// 	let old_verification_keys_set =
// old_verification_keys_vector.into_iter().collect::<BTreeSet<_>>();
// 	let mut old_did_details =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	old_did_details.verification_keys =
...
// 	assert_eq!(new_did_details.key_agreement_key,
// old_did_details.key_agreement_key); 	assert_eq!(new_did_details.
// delegation_key, old_did_details.delegation_key); 	assert_eq!(new_did_details.
// attestation_key, old_did_details.attestation_key); 	assert_eq!
// (new_did_details.endpoint_url, old_did_details.endpoint_url); 	assert_eq!
// (new_did_details.last_tx_counter, old_did_details.last_tx_counter + 1u64);
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let operation = generate_base_did_update_operation(BOB_DID);
...
// 	let mut mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	mock_did.last_tx_counter = u64::MAX;
...
// 	let mut mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	mock_did.last_tx_counter = 1;
// 	let mut operation = generate_base_did_update_operation(ALICE_DID);
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let mut operation =
// generate_base_did_update_operation(ALICE_DID); 	operation.tx_counter =
// mock_did.last_tx_counter;
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let operation = generate_base_did_update_operation(ALICE_DID);
...
// 	let key4 =
// did::PublicVerificationKey::from(get_sr25519_attestation_key(false).
// public()); 	let old_verification_keys = vec![key1, key2, key3];
// 	let old_verification_keys_ids = old_verification_keys.iter().map(|(i, _)| {
// 		did::utils::calculate_key_id(did::DidVerificationKeyType::AssertionMethod, 0)
// 	});

// 	let mut old_verification_keys_map = BTreeMap::<did::KeyId,
// did::PublicVerificationKey>::new(); 	for (key_id, key) in
// zip(old_verification_keys_ids, old_verification_keys).iter() {
// 		old_verification_keys_map.insert(key_id, key.clone());
// 	}
// 	let mut old_did_details =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	old_did_details.verification_keys = old_verification_keys_map;

// 	// Remove some verification keys including one not stored on chain (key4)
// 	let verification_keys_to_remove = vec![key2, key3, key4];
// 	let verification_keys_ids_to_remove =
// 	let mut operation = generate_base_did_update_operation(ALICE_DID);
// 	operation.verification_keys_to_remove =
// Some(verification_keys_to_remove.into_iter().collect());
...
// did::PublicVerificationKey::from(new_auth_key.public()));
...
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let mut operation =
// generate_base_did_delete_operation(ALICE_DID); 	operation.tx_counter =
// mock_did.last_tx_counter + 2;
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let operation = generate_base_did_delete_operation(ALICE_DID);
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let operation = TestDidOperation {
// 		did: ALICE_DID,
...
// Did::get_did(&operation.did).expect("DID should be present on chain."));
// 	assert_eq!(
...
// 	let mut mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	mock_did.attestation_key =
// Some(did::PublicVerificationKey::from(att_key.public())); 	let operation =
...
// 	let mut mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	mock_did.delegation_key =
// Some(did::PublicVerificationKey::from(del_key.public())); 	let operation =
// TestDidOperation { 		did: ALICE_DID,
...
// 	let did_details = ext.execute_with(||
// Did::get_did(&operation.did).expect("DID should be present on chain."));
// 	assert_eq!(did_details.get_tx_counter_value(),
// mock_did.get_tx_counter_value()); }
...
// 	let mock_did =
// generate_base_did_details(did::PublicVerificationKey::from(auth_key.
// public())); 	let verification_key_required =
// did::DidVerificationKeyType::CapabilityInvocation; 	let operation =
// TestDidOperation { 		did: ALICE_DID,
...
// 		"ftps://user@super.long.domain.kilt.io:12345/public/files/test.txt".
// as_bytes() 	));

// 	// All other valid ASCII characters
// 	assert_ok!(did::FtpUrl::try_from("ftps://:/?#[]@%!$&'()*+,;=-._~".
// as_bytes()));
...
// 	// Base32 address (at the moment, padding characters can appear anywhere in
// the 	// string)
// 	assert_ok!(did::IpfsUrl::try_from(
...
// 	assert_eq!(
// 		did::IpfsUrl::try_from("ipfs://¶
// QmdQ1rHHHTbgbGorfuMMYDQQ36q4sxvYcB4GDEHREuJQkL".as_bytes()), 		Err(did::
// UrlError::InvalidUrlEncoding) 	);

// 	assert_eq!(
// 		did::IpfsUrl::try_from("ipf://QmdQ1rHHHTbgbGorfuMMYDQQ36q4sxvYcB4GDEHREuJQkL"
// .as_bytes()), 		Err(did::UrlError::InvalidUrlScheme)
// 	);

// 	assert_eq!(
// 		did::IpfsUrl::try_from("ipfss://
// QmdQ1rHHHTbgbGorfuMMYDQQ36q4sxvYcB4GDEHREuJQkL".as_bytes()), 		Err(did::
