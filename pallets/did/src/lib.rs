...
//! # DID Pallet
//!
//! Provides W3C-compliant DID functionalities. A DID identifier is derived from a KILT address
//! and must be verifiable, i.e., must be able to generate digital signatures that can be verified
//! starting from a raw payload, its signature, and the signer identifier. Currently, the DID pallet
//! supports the following types of keys: Ed25519, Sr25519, and Ecdsa for signing keys, and X25519 for encryption keys.
//!
//! - [`Config`]
//! - [`Call`]
//! - [`Origin`]
...
//!
//! - One **authentication key**: used to sign and authorise DID-management operations (e.g., the update of some keys or the deletion of the whole DID).
//!
...
//!
//! - Zero or one **delegation key**: used to sign and authorise the creation of new delegation nodes on the KILT blockchain. In case no delegation key is present, the DID subject cannot write new delegations on the KILT blockchain. For more info, check the [delegation pallet](../../delegation/).
//!
//! - Zero or one **attestation key**: used to sign and authorise the creation of new attested claims on the KILT blockchain. In case no attestation key is present, the DID subject cannot write new attested claims on the KILT blockchain. For more info, check the [attestation pallet](../../attestation/).
//!
...
//!
//! - An optional **endpoint URL**: pointing to the description of the services the DID subject exposes. For more information, check the W3C DID Core specification.
//!
...
//! - `submit_did_create_operation` - Register a new DID on the KILT blockchain under the given DID identifier.
//! - `submit_did_update_operation` - Update any keys or the endpoint URL of an existing DID.
//! - `submit_did_delete_operation` - Delete the specified DID and all related keys from the KILT blockchain.
//! - `submit_did_call` - Proxy a dispatchable function for an extrinsic that expects a DID origin. The DID pallet verifies the signature and the nonce of the wrapping operation and then dispatches the underlying extrinsic upon successful verification.
//!
//! ## Assumptions
//!
//! - The maximum number of new key agreement keys that can be specified in a creation or update operation is bounded by `MaxNewKeyAgreementKeys`.
//! - The maximum number of new keys that can be deleted in an update operation is bounded by `MaxVerificationKeysToRevoke`.
//! - The maximum length in ASCII characters of the endpoint URL is bounded by `MaxUrlLength`.
