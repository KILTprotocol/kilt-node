...
fn coinbase_rewards_few_blocks_detailed_check() {
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_eq!(Balances::usable_balance(2), user_2);
			assert_eq!(Balances::usable_balance(3), user_3 + d_rewards.saturating_div(6));
			assert_eq!(Balances::usable_balance(4), user_4 + d_rewards.saturating_div(12));
			assert_eq!(Balances::usable_balance(5), user_5);
...
fn kick_candidate_with_full_unstaking() {
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			for block in 1u64..1u64.saturating_add(max_unstake_reqs.saturated_into()) {
...
fn api_get_staking_rates() {
	let stake: u128 = 100_000 * DECIMALS;
	ExtBuilder::default()
