...
impl<T: Config> OnRuntimeUpgrade for StakingPayoutRefactor<T> {
	...
	fn post_upgrade() -> Result<(), &'static str> {
		...
		for delegator in DelegatorState::<T>::iter_values() {
			assert!(
				delegator.owner.is_some(),
				"There exists a delegator without a collator in post migration!"
			);
			assert!(
...
fn migrate_delegators<T: Config>() -> Weight {
	...
	DelegatorState::<T>::translate_values(
		|old: DelegatorOld<T::AccountId, BalanceOf<T>, MaxCollatorsPerDelegator>| {
			...
			Some(Delegator {
				amount: old.total,
				// We only allowed one delegation per delegator, thus taking first element is sufficient
