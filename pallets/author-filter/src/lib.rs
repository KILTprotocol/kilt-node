...
//! Small pallet responsible determining which accounts are eligible to author
//! at the current block height.
//!
//! Currently this pallet is tightly coupled to our stake pallet, but this
//! design should be generalized in the future.
//!
//! Using a randomness beacon supplied by the `Randomness` trait, this pallet
//! takes the set of currently staked accounts from pallet stake, and filters
//! them down to a pseudorandom subset. The current technique gives no
//! preference to any particular author. In the future, we could
//! disfavor authors who are authoring a disproportionate amount of the time in
...
pub mod pallet {
	...
	use frame_support::{inherent::Vec, log, pallet_prelude::*, traits::Randomness};
	...
	// This code will be called by the author-inherent pallet to check whether the
	// reported author of this block is eligible at this height. We calculate that
	// result on demand and do not record it instorage (although we do emit a
	// debugging event for now). This implementation relies on the relay parent's
	// block number from the validation data inherent. Therefore this implementation
	// must not be used as a preliminary check (only final) Further, the validation
	// data inherent **must** be included before this check is performed. Concretely
	// the validation data inherent must be included before the author inherent.
	impl<T: Config> author_inherent::CanAuthor<T::AccountId> for Pallet<T> {
		fn can_author(account: &T::AccountId) -> bool {
			...
			for i in 0..num_eligible {
				...
				// - The relay parent block number so that the eligible authors at the next
				//   height change. Avoids liveness attacks from colluding minorities of active
				//   authors.
			...
			// <Pallet<T>>::deposit_event(Event::Filtered(our_height, relay_height,
			// eligible.clone()));
	...
	/// The percentage of active staked authors that will be eligible at each
	/// height.
	#[pallet::storage]
	...
	pub enum Event<T: Config> {
		/// The amount of eligible authors for the filter to select has been
		/// changed.
		EligibleUpdated(Percent),
		/// The staked authors have been filtered to these eligible authors in
		/// this block. This is a debugging and development event and should be
		/// removed eventually. Fields are: para block height, relay block
		/// height, eligible authors
