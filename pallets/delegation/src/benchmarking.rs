...
// fn add_root_delegation<T: Config>(number: u32) ->
// Result<(DelegationTriplet<T>, T::Hash), DispatchError> where
...
// 	ctype::Module::<T>::add(RawOrigin::Signed(root_acc.clone()).into(),
// ctype_hash)?; 	Module::<T>::create_root(RawOrigin::Signed(root_acc.clone()).
// into(), root_id, ctype_hash)?;
...
// ) -> Result<(sr25519::Public, T::AccountId, T::DelegationNodeId),
// DispatchError> where
// 	T::AccountId: From<sr25519::Public>,
...
// 		let delegation_id = generate_delegation_id::<T>(level *
// children_per_level.get() + c);

// 		// only set parent if not root
// 		let parent = parent_id_check::<T>(root_id, parent_id);

// 		// delegate signs delegation to parent
// 		let hash: Vec<u8> = Module::<T>::calculate_hash(delegation_id, root_id,
// parent, permissions).encode(); 		let sig: T::Signature =
// sp_io::crypto::sr25519_sign(KeyTypeId(*b"aura"), &delegation_acc_public,
// hash.as_ref()) 			.ok_or("Error while building signature of delegation.")?
// 			.into();
...
// 		first_leaf = first_leaf.or(Some((delegation_acc_public, delegation_acc_id,
// delegation_id))); 	}

// 	let (leaf_acc_public, leaf_acc_id, leaf_id) =
// 		first_leaf.expect("Should not be None due to restricting children_per_level
// to NonZeroU32");
...
// benchmarks! {
// 	where_clause { where T: core::fmt::Debug, T::Signature:
// From<sr25519::Signature>, T::AccountId: From<sr25519::Public>,
// T::DelegationNodeId: From<T::Hash> }
...
// 		let (root_acc, root_id, leaf_acc, leaf_id) = setup_delegations::<T>(r,
// ONE_CHILD_PER_LEVEL.expect(">0"), Permissions::DELEGATE)?; 		let root_acc_id:
// T::AccountId = root_acc.into(); 	}: _(RawOrigin::Signed(root_acc_id.clone()),
// root_id, r) 	verify {
// 		assert!(Root::<T>::contains_key(root_id));
// 		let root_delegation = Root::<T>::get(root_id).ok_or("Missing root
// delegation")?; 		assert_eq!(root_delegation.owner, root_acc_id);
// 		assert_eq!(root_delegation.revoked, true);

// 		assert!(Delegations::<T>::contains_key(leaf_id));
// 		let leaf_delegation = Delegations::<T>::get(leaf_id).ok_or("Missing leaf
// delegation")?; 		assert_eq!(leaf_delegation.root_id, root_id);
...
// 	add_delegation {
// 		// do setup
// 		let (_, root_id, leaf_acc, leaf_id) = setup_delegations::<T>(1,
// ONE_CHILD_PER_LEVEL.expect(">0"), Permissions::DELEGATE)?;
...
// 		let hash_root = Module::<T>::calculate_hash(delegation_id, root_id,
// parent_id, perm); 		let sig: T::Signature =
// sp_io::crypto::sr25519_sign(KeyTypeId(*b"aura"), &delegate_acc_public,
// hash_root.as_ref()).ok_or("Error while building signature of
// delegation.")?.into();

// 		let delegate_acc_id: T::AccountId = delegate_acc_public.into();
// 		let leaf_acc_id: T::AccountId = leaf_acc.into();
// 	}: _(RawOrigin::Signed(leaf_acc_id), delegation_id, root_id, parent_id,
// delegate_acc_id, perm, sig) 	verify {
...
// 	// complexitiy: O(h * c) with h = height of the delegation tree, c = max
// number of children in a level 	revoke_delegation_root_child {
// 		let r in 1 .. MAX_REVOCATIONS;
// 		let (_, root_id, leaf_acc, leaf_id) = setup_delegations::<T>(r,
// ONE_CHILD_PER_LEVEL.expect(">0"), Permissions::DELEGATE)?; 		let children:
// Vec<T::DelegationNodeId> = Children::<T>::get(root_id); 		let child_id:
// T::DelegationNodeId = *children.get(0).ok_or("Root should have children")?;
// 		let child_delegation = Delegations::<T>::get(child_id).ok_or("Child of root
// should have delegation id")?; 	}: revoke_delegation(RawOrigin::
// Signed(child_delegation.owner.clone()), child_id, r, r) 	verify {
// 		assert!(Delegations::<T>::contains_key(child_id));
// 		let DelegationNode::<T> { revoked, .. } =
// Delegations::<T>::get(leaf_id).ok_or("Child of root should have delegation
// id")?; 		assert_eq!(revoked, true);
...
// 		let r in 1 .. MAX_REVOCATIONS;
// 		let (root_acc, _, _, leaf_id) = setup_delegations::<T>(r,
// ONE_CHILD_PER_LEVEL.expect(">0"), Permissions::DELEGATE)?;
// 	}: revoke_delegation(RawOrigin::Signed(root_acc.clone().into()), leaf_id, r,
// r) 	verify {
// 		assert!(Delegations::<T>::contains_key(leaf_id));
// 		let DelegationNode::<T> { revoked, .. } =
// Delegations::<T>::get(leaf_id).ok_or("Child of root should have delegation
...
// 			assert_eq!(generate_delegation_id::<Test>(1),
// generate_delegation_id::<Test>(1)); 			assert_ne!(generate_delegation_id::
// <Test>(1), generate_delegation_id::<Test>(2)); 			let root =
// generate_delegation_id::<Test>(1); 			let parent =
// generate_delegation_id::<Test>(2); 			assert_eq!(parent_id_check::<Test>(root,
// root), None); 			assert_eq!(parent_id_check::<Test>(root, parent),
// Some(parent)); 		});
// 	}
// 	#[test]
...
// 				setup_delegations::<Test>(2, NonZeroU32::new(2).expect(">0"),
// Permissions::DELEGATE) 					.expect("failed to run delegation setup");
// 			assert!(Root::<Test>::contains_key(root_id));
