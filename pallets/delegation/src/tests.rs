...
fn check_submit_delegation_root_creation_operation_successful() {
	...
	let creator_did = did_mock::ALICE_DID;
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![(creator_did.clone(), delegator_details.clone())]);
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash, creator_did.clone())]);
...
fn check_did_max_tx_counter_submit_delegation_root_creation_operation() {
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![(creator_did.clone(), delegator_details.clone())]);
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash.clone(), creator_did.clone())]);
...
fn check_submit_delegation_no_parent_creation_operation_successful() {
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![
		(creator_did.clone(), delegator_details.clone()),
