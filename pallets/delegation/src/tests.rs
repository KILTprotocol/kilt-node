...
fn check_submit_delegation_root_creation_operation_successful() {
	...
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash, delegator_did.clone())]);
...
fn check_did_not_found_submit_delegation_root_creation_operation() {
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![(alternative_did, delegator_details.clone())]);
...
fn check_did_max_tx_counter_submit_delegation_root_creation_operation() {
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![(delegator_did.clone(), delegator_details.clone())]);
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash.clone(), delegator_did.clone())]);
...
fn check_did_delegation_key_not_present_submit_delegation_root_creation_operation() {
	...
	let delegator_details = did_mock::generate_mock_did_details(
		did::PublicVerificationKey::from(auth_key.public()),
		did::PublicEncryptionKey::from(enc_key),
	);
...
fn check_submit_delegation_no_parent_creation_operation_successful() {
	...
	let ctype_hash = ctype_mock::get_ctype_hash(true);
	let root_id = get_delegation_root_id(true);
	let parent_id: Option<TestDelegationNodeId> = None;
...
fn check_delegator_did_not_found_submit_delegation_with_parent_creation_operation() {
	...
	let builder = did_mock::ExtBuilder::default().with_dids(vec![
		(delegate_did.clone(), delegator_details.clone()),
		(alternative_did.clone(), delegate_details.clone()),
	]);
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash, alternative_did.clone())]);
	let builder = ExtBuilder::from(builder)
		.with_root_delegations(vec![(
			root_id,
			delegation::DelegationRoot {
...
fn check_delegator_delegation_key_not_present_submit_delegation_with_parent_creation_operation() {
	...
	// No delegation key specified for the creator DID.
...
fn check_delegate_did_not_found_submit_delegation_with_parent_creation_operation() {
	...
	// Only creator DID is added to mock storage
	let builder = did_mock::ExtBuilder::default().with_dids(vec![(delegator_did.clone(), delegator_details.clone())]);
	let builder = ctype_mock::ExtBuilder::from(builder).with_ctypes(vec![(ctype_hash, delegator_did.clone())]);
	let builder = ExtBuilder::from(builder)
		.with_root_delegations(vec![(
...
fn check_list_hierarchy_submit_delegation_root_revocation_operation_successful() {
	...
	let stored_delegation_root = ext
		.execute_with(|| Delegation::roots(&operation.root_id).expect("Delegation root should be present on chain."));
	assert_eq!(stored_delegation_root.revoked, true);

	let stored_delegation_1 = ext
		.execute_with(|| Delegation::delegations(&delegation_1_id).expect("Delegation 1 should be present on chain."));
	assert_eq!(stored_delegation_1.revoked, true);

	let stored_delegation_2 = ext
		.execute_with(|| Delegation::delegations(&delegation_2_id).expect("Delegation 2 should be present on chain."));
	assert_eq!(stored_delegation_2.revoked, true);
...
fn check_exact_children_max_revocations_submit_delegation_root_revocation_operation_successful() {
	...
	let stored_delegation_1 = ext
		.execute_with(|| Delegation::delegations(&delegation_1_id).expect("Delegation 1 should be present on chain."));
	assert_eq!(stored_delegation_1.revoked, false);
	...
	let stored_delegation_3 = ext
		.execute_with(|| Delegation::delegations(&delegation_3_id).expect("Delegation 3 should be present on chain."));
	assert_eq!(stored_delegation_3.revoked, false);
