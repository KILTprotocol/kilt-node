...
pub trait BondingFunction<O: Fixed, I> {
	...
	fn get_value(&self, x: I) -> Result<O, ArithmeticError>;

	/// calculates the cost of the curve between low and high
	fn calculate_costs(&self, low: I, high: I) -> Result<O, ArithmeticError> {
...
impl<O, I> BondingFunction<O, I> for PolynomialFunctionParameters<I> {
	/// F(x) = m * x^3 + n * x^2 + o * x
	fn get_value(&self, x: I) -> Result<O, ArithmeticError> {
		...
		ensure!(result >= O::ZERO, ArithmeticError::Underflow);
		Ok(result)
	}
}
