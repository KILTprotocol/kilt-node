...
use crate::{
	...
	mock::{runtime::*, Float},
	types::{convert_balance_to_parameter, Curve, DiffKind},
};

#[test]
fn test_mint_first_coin() {
	let a: u128 = Float::max_value().to_num();
	...
	let active_issuance_post: u128 = 10u128.pow(10);

	let q = active_issuance_post * active_issuance_post;
	...
	let costs = curve
		.calculate_cost(
			convert_balance_to_parameter::<Test>(active_issuance_pre).unwrap(),
			convert_balance_to_parameter::<Test>(active_issuance_post).unwrap(),
			convert_balance_to_parameter::<Test>(passive_issuance).unwrap(),
			DiffKind::Mint,
		)
		.unwrap();

	assert_eq!(costs, Float::from_num(4 * 10u128.pow(15)));
}
