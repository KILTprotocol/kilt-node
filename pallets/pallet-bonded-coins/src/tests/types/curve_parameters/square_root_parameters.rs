...
use crate::{
	curves_parameters::{BondingFunction, SquareRootBondingFunctionParameters},
	mock::Float,
};
...
fn test_basic() {
	...
	assert_eq!(result, Float::from_num(3));
}
...
fn test_fraction() {
	...
	let result = curve.get_value(x).unwrap();
	assert_eq!(result, Float::from_num(2.5));
}
...
fn test_large_values() {
	...
	let x = Float::from_num(1000000);
	// 1000000*sqrt(1000000^3) + 1000000*1000000 = 1001000000000000
	assert_eq!(params.get_value(x), Ok(Float::from_num(1001000000000000u128)));
}
...
fn test_negative_n() {
	...
	let x = Float::from(4);
	// 2*sqrt(4^3) - 3*4 = 2*sqrt(64) - 12 = 2*8 - 12 = 16 - 12 = 4
	assert_eq!(params.get_value(x), Ok(Float::from(4)));
}
...
fn test_precision_large_fraction() {
	...
	// = 1.999995500002874999 -> 1.9999955000028749

	let result = curve.get_value(x).unwrap();
	assert_eq!(result, Float::from_num(1.9999955000028749));
}
#[test]
fn test_precision_mixed_fraction() {
	let m = Float::from_num(0.3); // 0.3
	let n = Float::from_num(0.75); // 0.75
	let x = Float::from_num(0.5); // 0.5

	let curve = SquareRootBondingFunctionParameters { m, n };

	// 0.3*sqrt(0.5^3) + 0.75*0.5
	// = 0.481066017177982128 -> 481066017177982. Strange error here.

	let result = curve.get_value(x).unwrap();
	assert_eq!(result, Float::from_num(0.481066017177982));
}

#[test]
fn test_precision_small_fraction() {
	let m = Float::from_num(0.001); // 0.001
	let n = Float::from_num(0.001); // 0.001
	let x = Float::from_num(0.001); // 0.001

	let curve = SquareRootBondingFunctionParameters { m, n };

	// 0.001*sqrt(0.001^3) + 0.001*0.001
	// = 0.000001031622776601 -> 0.000001031622776

	let result = curve.get_value(x).unwrap();
	assert_eq!(result, Float::from_num(0.000001031622776));
}
